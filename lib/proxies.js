 ...
 */
const pageProxy = (page, options = {}) => {
  return new Proxy(page, new PageHandler(options));
};
...
class BrowserHandler extends BaseHandler {
  ...
  async newPage(browser) {
    ...
    const page = pageProxy(rawPage, this.__pageOptions__);
  ...
  async throttle(options) {
    let throttle = options.throttle || this.__pageOptions__.throttle;
  ...
  async pages(browser) {
    ...
    return rawPages.map(rawPage => pageProxy(rawPage, this.__pageOptions__));
  }
  ...
  async currentPage(browser) {
    ...
    if (rawPages.length > 0) {
      page = pageProxy(rawPages[rawPages.length - 1], this.__pageOptions__);
    }
    return page;
  }
