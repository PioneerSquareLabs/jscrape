...
class PageHandler extends BaseHandler {
  ...
  async clickAndNavigate(page, elementHandle) {
    ...
    try {
      console.error(
        `jscrape: DEBUG: clickAndNavigate: waitForNavigation with options: ${JSON.stringify(
          this.__navigationOptions__
        )}`
      );
      const [response] = await Promise.all([
        page.waitForNavigation(this.__navigationOptions__).catch(error => {
          console.log(
            "jscrape: clickAndNavigate: fail waitForNavigation: ",
            error
          );
        }),
        elementHandle.click().catch(error => {
          console.log("jscrape: clickAndNavigate: fail click: ", error);
        })
      ]);
      success = true;
      console.error(
        `jscrape: DEBUG: clickAndNavigate: finished waitForNavigation with options: ${JSON.stringify(
          this.__navigationOptions__
        )}`
      );
    } catch (error) {
